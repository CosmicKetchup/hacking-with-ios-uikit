...
final class PetitionTableViewController: UITableViewController {
    ...
    private func setupView() {
        ...
        navigationItem.title = "Project 07"
...
extension PetitionTableViewController {
    fileprivate func loadPetitions() {
        if let targetUrl = URL(string: searchType.address), let rawData = try? Data(contentsOf: targetUrl) {
            let decoder = JSONDecoder()
            guard let jsonPetitions = try? decoder.decode(Petitions.self, from: rawData) else { showConnectionError(); return }
